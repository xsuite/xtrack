...
@pytest.mark.parametrize("qx_shift", [-0.015, 0.035], ids=lambda v: f"qx_shift={v}")
@pytest.mark.parametrize("qy_shift", [0.015, -0.02], ids=lambda v: f"qy_shift={v}")
def test_ac_dipole_madng_twiss(test_context: Any, qx_shift: float, qy_shift: float):
    """Test the MAD-NG conversion of thin AC dipoles on the tune shift."""
    n = 4  # Number of FODO cells
    fodo = [
        xt.Multipole(length=0.2, knl=[0, +0.2], ksl=[0, 0]),
        xt.Drift(length=1.0),
    ...
    line.insert("e5_hacd", at='e5')
    line.insert("e5_vacd", at='e5')

    line.build_tracker(_context=test_context)
    tws_both = line.twiss(method="4d")
    tws_both_madng = line.madng_twiss()

    xo.assert_allclose(tws_both["qx"], drv_qx, rtol=1e-10, atol=1e-15)
    xo.assert_allclose(tws_both["qy"], drv_qy, rtol=1e-10, atol=1e-15)
    xo.assert_allclose(tws_both_madng.mu1_ng[-1], drv_qx, rtol=1e-10, atol=1e-15)
    xo.assert_allclose(tws_both_madng.mu2_ng[-1], drv_qy, rtol=1e-10, atol=1e-15)
    xo.assert_allclose(tws_both_madng.mu1_ng[-1], tws_both["qx"], rtol=1e-10, atol=1e-15)
    xo.assert_allclose(tws_both_madng.mu2_ng[-1], tws_both["qy"], rtol=1e-10, atol=1e-15)
