...
def extract_qs_spacings(f, psd, window_width=0.15, threshold=0.01):
    # Extract the distance between succesive peaks in the spectrum
...
def test_qs_qx_qy_linear_rf():
	...
	lmap = xt.LineSegmentMap(length=26658.8831999989, qx=0.27, qy=0.295, dqx=15, dqy=15, 
						  longitudinal_mode='linear_fixed_qs', qs=0.004, bets=1, betx=1, bety=1)
	...
	schottky_monitor.process_spectrum(inst_spectrum_len=10000, delta_q=5e-5, band_width=0.45, qx=line.elements[0].qx, qy=line.elements[0].qy)

	# Test syncrotron tune on each band
	for region in ['lowerH', 'center', 'upperH', 'lowerV', 'upperV']:
		spacings = extract_qs_spacings(schottky_monitor.frequencies[region], schottky_monitor.PSD_avg[region])
		# Assert that each value in spacings is close to line.elements[0].qs or an integer multiple of it (some peaks might have a height of 0)
	...
	# Test vertical and hoirizontal betatron tunes
	for region in ['lowerH', 'upperH', 'lowerV', 'upperV']:
		...
		f_win = f[mask]
		psd_win = psd[mask]
		# Check that full band is used to compute com and no overlap with band from adjacent harmonics
...
# To Do: add test for non-linear RF