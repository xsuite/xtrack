...
double synrad_gen_photon_energy_normalized(LocalParticle *part)
{
  ...
  do {
    if (RandomGenerator_get_double(part) < ratio) { // use low energy approximation
      result=c1+(1.-c1)*RandomGenerator_get_double(part);
      double tmp = result*result;
      result*=tmp;  	// take to 3rd power;
      exact=SynRad(result);
      appr=a1/tmp;
    } else {				// use high energy approximation
      result=xlow-log(RandomGenerator_get_double(part));
      exact=SynRad(result);
      appr=a2*exp(-result);
    }
  } while (exact < appr*RandomGenerator_get_double(part));	// reject in proportion of approx
...
int64_t synrad_emit_photons(LocalParticle *part, double curv /* 1/m */,
    ...
    double n = RandomGenerator_get_double_exp(part); // path_length / mean_free_path;
    while (n < synrad_average_number_of_photons(beta0 * gamma0, curv, lpath)) {
        ...
        n += RandomGenerator_get_double_exp(part);
        if (record){
