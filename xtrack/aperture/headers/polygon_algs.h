...
static inline float_type polygon_signed_area(const Point2D *poly, int len_poly)
{
    if (len_poly < 3)
        return 0.f;

    const int num_unique_points = point2d_distance(poly[0].x, poly[0].y, poly[len_poly - 1].x, poly[len_poly - 1].y) <= APER_PRECISION
        ? len_poly - 1
        : len_poly;

    float_type twice_area = 0.f;
    for (int i = 0; i < num_unique_points; i++)
    {
        const Point2D a = poly[i];
        const Point2D b = poly[(i + 1) % num_unique_points];
...
static inline char polygon_is_convex(const Point2D *poly, int len_poly)
{
    if (len_poly < 4)
        return 1;
    ...
    for (int i = 0; i < num_unique_points; i++)
    {
        const Point2D a = poly[i];
        const Point2D b = poly[(i + 1) % num_unique_points];
        const Point2D c = poly[(i + 2) % num_unique_points];
        const Point2D ab = point2d_sub(b, a);
        const Point2D bc = point2d_sub(c, b);
        const float_type cross = point2d_cross(ab, bc);

        if (fabs(cross) <= APER_PRECISION * APER_PRECISION)
            continue;

        const int this_sign = cross > 0 ? 1 : -1;
        if (sign == 0)
            sign = this_sign;
