...
void segments_from_polygon(const Point2D *vertices, int len_vertices, Segment2D *out_segments, int *out_len);
...
void segments_from_polygon(const Point2D *vertices, int len_vertices, Segment2D *out_segments, int *out_len)
/* Create a path for a polygon described by its unique vertices in order. */
{
    if (len_vertices < 2)
    {
        *out_len = 0;
    ...
    for (int i = 0; i < len_vertices; i++)
    {
        ...
        const Point2D end = vertices[(i + 1) % len_vertices];
        if (point2d_distance(start.x, start.y, end.x, end.y) <= APER_PRECISION)
            continue;

        line_segment_from_start_end(start.x, start.y, end.x, end.y, &out_segments[idx]);
        idx++;
    }
