...
GPUFUN
int64_t BeamStatsMonitor_find_selected_slot(
        BeamStatsMonitorData el,
        int64_t const slot
) {
    int64_t const n_selected =
        BeamStatsMonitorData_get__num_selected_slots(el);
    for (int64_t ii = 0; ii < n_selected; ii++) {
        if (BeamStatsMonitorData_get__selected_slots(el, ii) == slot) {
            return ii;
        }
    }
    return -1;
                    ...
                    if (mode > 0) {
                        ...
                        if (bunch_spacing_zeta != 0.0) {
                            slot = -floor((zeta - z_min_edge)
                                          / bunch_spacing_zeta);
                            i_selected =
                                BeamStatsMonitor_find_selected_slot(el, slot);
