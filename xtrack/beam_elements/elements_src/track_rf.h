...
void track_rf_kick_single_particle(
    ...
    double const energy_kick = weight * q * voltage
        * sin(DEG2RAD * (lag + lag_taper) - (2.0 * PI) / C_LIGHT * freq * tau);

    rfmultipole_energy_kick = 0;
    if (order >= 0) {
        ...
        for (int64_t kk = 0; kk <= order; kk++)
        {
            ...
            double const pn_kk = DEG2RAD * pn[kk] - (2.0 * PI) / C_LIGHT * freq * tau;
            double const ps_kk = DEG2RAD * ps[kk] - (2.0 * PI) / C_LIGHT * freq * tau;
        ...
        rfmultipole_energy_kick = - q * ( (k * p0c) * dptr );
