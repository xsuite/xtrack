#ifndef create_fieldexpansion_bent_H
#define create_fieldexpansion_bent_H
...
void build_expansion_bent(BentFieldExpansionData el){
    const double h  = BentFieldExpansionData_get_h(el);
    const int ncoef = BentFieldExpansionData_get__ncoef(el);
    const int na    = BentFieldExpansionData_get_na(el);
    const int nb    = BentFieldExpansionData_get_nb(el);
    const int deg   = BentFieldExpansionData_get_deg(el);
    double a[na * (deg + 1)];
    for (int i = 0; i < na*(deg+1); ++i){
        a[i] = BentFieldExpansionData_get_a(el,i);
    }
    double b[nb * (deg + 1)];
    for (int i = 0; i < nb*(deg+1); ++i){
        b[i] = BentFieldExpansionData_get_b(el,i);
    }
    double bs[deg + 1];
    for (int i = 0; i < deg + 1; ++i){
        bs[i] = BentFieldExpansionData_get_bs(el,i);
    }

    const int mmax = BentFieldExpansionData_get__mmax(el);
    const int mmin = BentFieldExpansionData_get__mmin(el);
    const int moff = BentFieldExpansionData_get__moff(el);
    const int nm   = BentFieldExpansionData_get__nm(el);

    double *c = (double *) BentFieldExpansionData_getp__c(el);
