"""Twiss, survey, and 100 particles tracked for 100 turns in the HELICS model.

Run from the repository root with::

    %run examples/bfieldexpansion/helics_twiss_survey_tracking.py
    python examples/bfieldexpansion/helics_twiss_survey_tracking.py --no-plot --save-prefix /tmp/helics

The lattice and fabricated tanh dipole coefficients are all in helics.py.
The beam is an on-momentum, matched Gaussian distribution with geometric rms
emittances of 1 mm mrad in both planes, for He2+ at 20 MeV kinetic energy.
Tracking uses pkin_const=False.
...
from helics import CIRCUMFERENCE, DIPOLE_SEGMENTS, make_line
...
def make_plots(line, twiss, survey, monitor, survived):
    ...
    axes[0].set(ylabel=r'$\beta$ [m]',
                title=f'HELICS fabricated tanh optics: Qx={twiss.qx:.5f}, Qy={twiss.qy:.5f}')
    ...
    ax.plot([], [], color='C0', lw=3, label='Dipole tanh spline segments')
    ...
    ax.set(xlabel='Z [m]', ylabel='X [m]',
           title=f'HELICS survey: circumference {CIRCUMFERENCE:.6f} m')
    ax.set_aspect('equal')
    ax.grid(alpha=0.25)
    ax.legend(loc='best')
    figures['survey'] = fig
