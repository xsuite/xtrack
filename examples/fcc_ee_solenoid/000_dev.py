...
opt_orbit_left = line.match(
    ...
    vary=xt.VaryList(h_correction_knobs_left + v_correction_knobs_left, step=1e-8)
)
opt_orbit_left.solve()

opt_orbit_right = line.match(
    name='orbit_correction_right',
    solve=False,
    start=ip_name,
    end=tt_right.name[-1],
    init_at=ip_name,
    ...
    vary=xt.VaryList(h_correction_knobs_right + v_correction_knobs_right, step=1e-8)
)
...
for nn in elements_for_coupling_correction_right:
    ...
    env[nn_coupling] = 0
    env[nn].ksl[1] = nn_coupling
    coupling_correction_knobs_right.append(nn_coupling)

opt_coupling_left = line.match(
    name='coupling_correction_left',
    solve=False,
    start=tt_left.name[0],
    end=ip_name,
    init_at=ip_name,
    init=tw_off,
    targets=xt.TargetRmatrix(
...
opt_orb_coupling_left = opt_orbit_left.clone(
    ...
    add_targets=opt_coupling_left.targets,
    add_vary=opt_coupling_left.vary
)
opt_orb_coupling_left.solve()

opt_orb_coupling_right = opt_orbit_right.clone(
    name='orb_coupling_correction_right',
    add_targets=opt_coupling_right.targets,
    add_vary=opt_coupling_right.vary
)
