...
import xtrack as xt
from xtrack._temp.boris_and_solenoid_map.solenoid_field import SolenoidField, Multifield
...
# sfm = SolenoidField(L=2, a=0.10, B0=2, z0=5)
# sfa1 = SolenoidField(L=1, a=0.10, B0=-4, z0=5-1.5)
# sfa2= SolenoidField(L=1, a=0.10, B0=-4, z0=5+1.5)
# sf = Multifield([sfa1, sfm, sfa2])

sfm = SolenoidField(L=4, a=0.10, B0=2, z0=0)
sf = Multifield([sfm])
...
z_check = sfm.z0 + sfm.L * np.linspace(-5, 5, 1001)
...
plt.figure(2)
axd1 = plt.subplot(2, 1, 1, sharex=ax1)
axd1.plot(mon.s[0, :], mon.y[1, :]*1e6)
plt.grid()

axd2 = plt.subplot(2, 1, 2, sharex=ax1)
axd2.plot(mon.s[0, :], 1e6*(mon.y[1, :] - mon.y[0, :])/(mon.delta[1, :] - mon.delta[0, :]))
axd2.plot(z_log[:, 0], 1e6*(y_log[:, 1] - y_log[:, 0])/(p0.delta[1] - p0.delta[0]), '--')
plt.grid()
...
axd2.set_ylabel(r'$D_y$ [$\mu m$]')
axd1.set_ylabel(r'$y$ [$\mu m$]')
