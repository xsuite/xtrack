...
INPUT_LATTICE_JSON = 'temp_fcc_ee_lcc_non_local_boris_solenoid.json'
OUTPUT_LATTICE_JSON = 'fccee_z_lcc_non_local_boris_solenoid.json'
...
def measure_ksol_l_main_solenoid(line, env, ip_name):
    ...
    tt_sol_doublet = line.get_table().rows[
        'dy_match_l_' + ip_name: 'dy_match_r_' + ip_name]

    for nn in tt_sol_doublet.name:
        element_type = tt_sol_doublet['element_type', nn]
        ee = env.get(tt_sol_doublet['env_name', nn])

        if element_type == 'VariableSolenoid':
            ksol_l += ee.ks_profile.mean() * ee.length
        elif element_type == 'SplineBoris':
            ksol_l += ee.scale_b * ee.bs[4] * ee.length / rigidity0
...
optimizers = {}
for ip_name in ['ipa']: #config.keys():
    ...
    # Switch on solenoid and compensation solenoid for this IP.
    ...
    # Measure integrated main-solenoid field and rotate the final doublets.
    ksol_l_main_solenoid = measure_ksol_l_main_solenoid(line, env, ip_name)

    # In parentheses is the full solenoid rotation; we want half of it for
    # each doublet.
    ...
    # Define orbit corrector knobs. The first pair is already embedded in the
    # solenoid region by 005d.
    ...
    env['corr_sol_right_' + ip_name].knl[0] += (
    ...
    env['corr_sol_left_' + ip_name].knl[0] += (
    ...
    env['corr_sol_right_' + ip_name].ksl[0] += (
    ...
    env['corr_sol_left_' + ip_name].ksl[0] += (
