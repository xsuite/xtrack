...
line = xt.load('psb_00_from_mad.json')
line.build_tracker()
...
tw = line.twiss()
assert np.isclose(tw['x', 'bi1.tstr1l1'], -0.045716, rtol=0, atol=1e-4)
assert np.isclose(tw['y', 'bi1.tstr1l1'], 0.0000000, rtol=0, atol=1e-4)
...
tw = line.twiss()
assert np.isclose(tw['x', 'bi1.tstr1l1'], -0.0458633, rtol=0, atol=1e-4)
assert np.isclose(tw['y', 'bi1.tstr1l1'], 0.0000000, rtol=0, atol=1e-4)
assert np.isclose(tw['betx', 'bi1.tstr1l1'], 5.266456, rtol=1e-3)
