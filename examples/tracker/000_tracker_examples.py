...
assert tracker.record_last_track.x.shape == (3,3)

### Behavior:

# For num_turns = 1 (default):
#  - tracker.track(particles, ele_start=3, ele_stop=5) tracks once from element 3 to
#    element 5 e. particles.at_turn is not incremented as the particles never pass through
#    the end of the beam line.
#  - tracker.track(particles, ele_start=7, ele_stop=3) tracks once from element 5 to
...
p = xp.Particles(x=[1e-3, 2e-3, 3e-3], p0c=7e12, s=5 * 2., at_element=5)
tracker.track(p, num_turns=1, ele_start=5, ele_stop=2,
              turn_by_turn_monitor=True)
...
# 0 <= ele_start < ele_stop -- num_turns > 1
p = xp.Particles(x=[1e-3, 2e-3, 3e-3], p0c=7e12, s=5 * 2., at_element=5)
tracker.track(p, num_turns=4, ele_start=5, ele_stop=2,
              turn_by_turn_monitor=True)
