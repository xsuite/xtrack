...
""")
    ...
    for ss in slices:
        mad['tbl', 'flw'] = mad.track(
            sequence='seq', observe=0, save='"atall"', X0={'x': 0},
...
title = f'length={length:.3f} angle={angle:.3g}, k2l={k2l:.3g}, x_in={0:.3e}'

plt.close('all')
plt.figure(1)
plt.loglog(slices, np.abs(np.array(x_list)),
           'x-', label='MAD-NG')
for model, x_list_xt in xsuite_x_by_model.items():
    plt.loglog(slices, np.abs(np.array(x_list_xt)), '.-',
