...
plt.figure(10)
beta0 = tw.particle_on_co.beta0[0]
plt.loglog(slices, beta0 * np.abs(np.array(t_list) - t_list[-1]),
           'x-', label='MAD-NG')
for model, zeta_list_xt in xsuite_zeta_by_model.items():
    plt.loglog(slices, np.abs(np.array(zeta_list_xt) - zeta_list_xt[-1]), '.-',
               label=f'Xsuite {model}')
plt.xlabel('Number of slices')
plt.ylabel('|zeta - zeta(last slice)|')
plt.legend()
plt.suptitle(title)
