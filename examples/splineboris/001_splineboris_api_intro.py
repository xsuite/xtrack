...
`SplineBoris.get_field` expects the local longitudinal coordinate
``s_local \in [0, length]``.
"""

# 1) Bx, By, Bs as a function of s_local at (x, y) = (0, 0)
s_grid = np.linspace(0, sb.length, 200)
Bx_s = []
By_s = []
Bs_s = []
for s_local in s_grid:
    Bx_val, By_val, Bs_val = sb.get_field(0.0, 0.0, s_local)
    Bx_s.append(Bx_val)
    By_s.append(By_val)
    Bs_s.append(Bs_val)
...
ax.set_xlabel("x [mm]")
ax.set_ylabel("By [T]")
ax.set_title(f"By(x) at s_local = {s_mid:.3f} m")
ax.grid(True, alpha=0.3)
