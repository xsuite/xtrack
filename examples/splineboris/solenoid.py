...
integrator_splineboris = xt.BorisSpatialIntegrator(fieldmap_callable=sf.get_field,
                                        s_start=0,
                                        s_end=30,
                                        n_steps=15000)
p_splineboris = p.copy()
integrator_splineboris.track(p_splineboris)

# Construct field map:
x_axis = np.linspace(-1, 1, 5)
y_axis = np.linspace(-1, 1, 5)
z_axis = np.linspace(0, 30, 1001)

X, Y, Z = np.meshgrid(x_axis, y_axis, z_axis, indexing="ij")

# Evaluate field on every grid point
Bx, By, Bz = sf.get_field(X.ravel(), Y.ravel(), Z.ravel())
Bx = Bx.reshape(X.shape)
By = By.reshape(X.shape)
Bz = Bz.reshape(X.shape)

# Optional: write solenoid_field.dat with one row per grid point
...
# Fit field map:
fitter = FieldFitter(df_raw_data=data,
    xy_point=(0, 0),
    dx=0.001,
    dy=0.001,
    ds=0.001,
...
fitter.set()
fitter.save_fit_pars(field_maps_dir / "solenoid_fit_pars.csv")
...
#                     rtol=0, atol=np.max(np.abs(ay_ref)*3e-2))p0 = xt.Particles(mass0=xt.ELECTRON_MASS_EV, q0=1,
#                 energy0=45.6e9/1000,
#                 x=[-1e-3, -1e-3],
#                 px=-1e-3*(1+delta),
#                 y=1e-3,
