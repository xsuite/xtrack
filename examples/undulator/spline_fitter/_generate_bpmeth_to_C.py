...
def make_symbols(multipole_order=multipole_order, poly_order=4):
    ...
    for i in range(multipole_order):
        for k in range(poly_order+1):
            ks_symbol = sp.symbols(f'ks_{i + 1}_{k}')
            kn_symbol = sp.symbols(f'kn_{i + 1}_{k}')
...
def param_names_list(multipole_order=multipole_order, poly_order=4):
    ...
    for i in range(multipole_order):
        for k in range(poly_order+1):
            ks_name = f'ks_{i + 1}_{k}'
            kn_name = f'kn_{i + 1}_{k}'
...
def write_to_C_scalar(max_order=multipole_order, poly_order=4, field='B'):
    ...
    if field == 'A':
        filename = '_spline_A_field_eval_scalar.h'
    else:
        filename = '_spline_B_field_eval_scalar.h'
