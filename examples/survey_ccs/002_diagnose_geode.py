...
def main():
    ...
    misalignments = pd.read_csv(ROOT / 'bump_misalignments.csv', index_col=0)
    ...
    new_socket = read_geode('geode_socket_point_xsuite_no_bumps.csv')
    ...
    aligned_line = xt.Line.from_json(ROOT / 'h4_misaligned.json')
    aligned = survey(aligned_line)
    # The reader now supplies the correct geometric radial sign. The saved
    # JSON/TFS and GEODE snapshots predate that fix. Leave an unreported exit's
    # transverse bumps at zero for this additional diagnostic hypothesis.
    ...
    exported = tfs.read(ROOT / 'h4_survey_comp_vbend_output.tfs')
    exported_points = {}
    ...
    beam.to_csv(ROOT / 'geode_diagnostic_beam.csv')
    ...
    for point in old_socket.index.intersection(new_socket.index):
        ...
        request = np.zeros(3) if req is None else np.array([
            ...
            for axis in 'rst'])
        # Express the prediction in the raw report convention.
        request[0] *= -1
        radial_prediction = basis[:, 0] * (2*request[0])
        residual = delta-radial_prediction
        ...
        row = {'point': point, 'name': name,
               ...
               'after_radial_sign_norm_mm': np.linalg.norm(residual)*1000,
               'after_rigid_correction_norm_mm': np.linalg.norm(corrected_residual)*1000,
               ...
               'request_r_at_socket_mm': request[0]*1000,
        ...
        for axis, value in zip('xyz', residual):
            row[f'after_radial_sign_{axis}_mm'] = value*1000
        for axis, value in zip('xyz', corrected_residual):
            row[f'after_rigid_correction_{axis}_mm'] = value*1000
    ...
    sockets.to_csv(ROOT / 'geode_diagnostic_socket.csv')
    ...
    cols = ['name', 'norm_mm', 'after_radial_sign_norm_mm',
            'after_rigid_correction_norm_mm', 'after_seed_correction_norm_mm',
    ...
    print('\nLargest residuals after sign, missing-exit and seed corrections:')
