...
def bmad_quadrupole(k1, p0c, delta, length, spin_test, px=0, py=0):
    ...
    p = p0.copy()
    magnet = xt.Magnet(h=0, k1=k1, length=length)
    magnet.track(p)
...
out_on_mom = bmad_quadrupole(k1=k1, p0c=p0c, delta=0,
                         length=length, spin_test=spin_test)
out_off_mom_p0c = bmad_quadrupole(k1=k1, p0c=p0c*(1 + delta), delta=0,
                              length=length, spin_test=spin_test)
out_off_mom_delta = bmad_quadrupole(k1=k1, p0c=p0c, delta=delta,
                                length=length, spin_test=spin_test)
out_off_mom_pxpy = bmad_quadrupole(k1=k1, p0c=p0c, delta=0,
                                length=length, spin_test=spin_test,
                                px=1e-2, py=2e-3)
...
for dd in delta_vect:
    print('dd', dd)
    out = bmad_quadrupole(k1=k1, p0c=p0c, delta=dd, length=length, spin_test=spin_test)
...
for px, py in zip(px_vect, py_vect):
    print('px', px)
    out = bmad_quadrupole(k1=k1, p0c=p0c, delta=1e-3, length=length, spin_test=spin_test,
                        px=px, py=py)
